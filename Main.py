...
from mindspore.nn import Linear, Conv1d
import numpy as np
...
def logger(info, model, optimizer):
    ...
    if type(epoch) == int and epoch % args.save_interval == 0:
        ...
        if model is not None:
            # print("This is torch.save(model.state_dict(), model_name)")
            torch.save(model.state_dict(), model_name)
        if optimizer is not None:
            # print("This is torch.save(optimizer.state_dict(), optimizer_name)")
            torch.save(optimizer.state_dict(), optimizer_name)


# Arguments
parser = argparse.ArgumentParser(description='LGCDA')
# general settings
...
parser.add_argument('--data-name', default='mydataset', help='dataset name')


parser.add_argument('--data-appendix', default='_mnph200',
                    help='what to append to save-names when saving datasets')
parser.add_argument('--save-appendix', default='_mnph200',
...
parser.add_argument('--ARR', type=float, default=0,
...
parser.add_argument('--num-relations', type=int, default=1,
...
if __name__ == '__main__':
    ...
    (
        ...
        test_v_indices, trainall_labels, trainall_u_indices, trainall_v_indices, class_values,
        circRNA_similarity, disease_similarity
    ) = mydataset_create_trainvaltest_split(
    ...
    trainall_indices = (trainall_u_indices, trainall_v_indices)
    # print('#train: %d, #val: %d, #test: %d, #trainall: %d' % (
    #     len(train_u_indices),
    #     len(val_u_indices),
    #     len(test_u_indices),
    #     len(trainall_u_indices),
    # ))
    ...
    if args.visualize:
        ...
        if args.transfer:
            test_once(test_graphs, model, args.batch_size, logger)
    else:
        ...
        if args.ensemble:
            ...
            if args.transfer:
                ...
                epoch_info = 'transfer {}, ensemble of range({}, {}, {})'.frmat(
            ...
            if args.transfer:
                model.load_state_dict(torch.load(args.model_pos))
                test_once(test_graphs, model, args.batch_size, logger=None)
